...
function signUp($name, $surname, $email, $gender, $birthDate, $password){
	...
	if($stmt->execute()){
	  ...
	  if($stmt->fetch()){
		...
		if(password_verify($password, $passwordFromDb)){
		  ...
		  $stmt->close();
		  $stmt = $conn->prepare("SELECT bgcolor, txtcolor FROM userprofiles WHERE userid=?");
		  echo $conn->error;
  ...
  function storeuserprofile($description, $bgColor, $txtColor){
	$notice = "";
	$conn = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
    $stmt = $conn->prepare("SELECT id FROM userprofiles WHERE userid=?");
	echo $conn->error;
	$stmt->bind_param("i", $_SESSION["userId"]);
	...
	if($stmt->fetch()){
		...
		$stmt->close();
		$stmt = $conn->prepare("UPDATE userprofiles SET description = ?, bgcolor = ?, txtcolor = ? WHERE userid=?");
		echo $conn->error;
		...
		$stmt->close();
		$stmt = $conn->prepare("INSERT INTO userprofiles (userid, description, bgcolor, txtcolor) VALUES(?,?,?,?)");
		echo $conn->error;
  ...
  function showMyDesc(){
	$notice = null;
	$conn = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
	$stmt = $conn->prepare("SELECT description FROM userprofiles WHERE userid=?");
	echo $conn->error;
	$stmt->bind_param("i", $_SESSION["userId"]);
  ...
  function lisauudis($userid, $newsheadline, $newsbody, $added_date){
	$notice = null;
	$conn = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
	$stmt = $conn->prepare("INSERT INTO news (userid, title, content, added) VALUES(?,?,?,?)");
	echo $conn->error;
	
	$stmt->bind_param("isss", $userid, $newsheadline, $newsbody, $added_date);
	
	if($stmt->execute()){
		$notice = "Uudise lisamine õnnestus!";
	} else {
		$notice = "Uudise lisamisel tekkis tehniline tõrge: " .$stmt->error;
	}
